function [F] = F_Fourier_Chebyshev_backward(c,ta,b,theta,h,N,n,m)
...
for k = -m:m
    ...
    F(ind+1,k+m+1) = 2*ind.*c(ind+1,k+m+1) - (-lambda_k*c(ind,k+m+1)+lambda_k*c(ind+2,k+m+1)+h*exp(1i*theta)*(weight.*ta_c(ind+2,k+m+1)-ta_c(ind,k+m+1)));
end
