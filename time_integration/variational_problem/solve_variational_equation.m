function [C,r_minus] = solve_variational_equation(a,theta,h,N,n,m,rigorous)
...
DF = DF_Fourier_Chebyshev(ta,theta_mid,h,N,n,m);
...
for j=1:2*m+1
    ...
    F = F_Fourier_Chebyshev(c,ta,b,theta_mid,h,N,n,m);
    itercount=0;
...
if rigorous>0
  [r_minus,success] = int_radii_polynomial_variational_equation(C,ta,theta,h,N,n,m,n_pad,nu);
else
  [r_minus,success] = radii_polynomial_variational_equation(C,ta,theta,h,N,n,n_pad,nu,m);
end
% success
if success~=1
  disp('variational problem is not verified!')
  r_minus = nan;
else
  disp('variational problem: success')
end