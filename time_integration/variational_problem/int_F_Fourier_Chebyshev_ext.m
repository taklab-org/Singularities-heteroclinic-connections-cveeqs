function [F] = int_F_Fourier_Chebyshev_ext(c,ta,b,theta,h,N,n,m)
...
for k = -N:N
    ...
    F(ind+1,k+N+1) = -lambda_k*c(ind,k+N+1)+2*ind.*c(ind+1,k+N+1)+lambda_k*c(ind+2,k+N+1)+h*exp(1i*theta)*weight.*(ta_c(ind+2,k+N+1)-ta_c(ind,k+N+1));
end
