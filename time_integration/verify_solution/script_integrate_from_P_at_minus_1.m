...
%   if timestep==1
    % Initial error
    if rigorous>0
      eps_all = sup(eps_all + compute_eps(intval(a),intval(a0))) + 1e-10;
...
%   end
  ...
  if rigorous>0
    ...
    alp_N = (core+1)^2*(2*ipi)^2*cos(itheta);
    a_X = a_norm(intval(a));
    a_infty = a; a_infty(:,N+1) = 0;
    a_inf_X = a_norm(intval(a_infty));
  else
    theta = mid(theta);
    alp_N = (core+1)^2*(2*pi)^2*cos(theta);
    a_X = a_norm(a);
    a_infty = a; a_infty(:,N+1) = 0;
...
% save data_CGL_from_P_at_minus_1.mat
